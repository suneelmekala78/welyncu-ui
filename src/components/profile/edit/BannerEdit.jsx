  ...
  const handleSave = async () => {
    ...
    try {
      const res = await apiRequest({
        url: "/user/add-banner-pic",
        data: formData,
        method: "POST",
      });

      if (res?.status === "success") {
        setPreviewUrl(res.bannerUrl); // Set the new image from the server
      ...
      <div className="popup-section popup-section-active">
        <div className="popup-container box-shadow p-15">
          ...
          <div className="profile-pic-edit-section">
            <div className="profile-pic-edit-top banner-edit-top">
              <img
                src={previewUrl ? previewUrl : user?.bannerUrl}
                alt="banner-pic"
              />
            </div>
